...
class PrimitiveTree(list):
    ...
    """
    def __init__(self, content): #list型に与えられたcontentを入れているよく使う書き方
        list.__init__(self, content)

    def __deepcopy__(self, memo):#自分と同じオブジェクトを渡すだけの物、これがないとdeepcopyは使えない
        new = self.__class__(self) #型コピー
        new.__dict__.update(copy.deepcopy(self.__dict__, memo)) #なにも入ってない自分のクラスに自分が持っているdictを渡す
        return new

    def __setitem__(self, key, val):
        # Check for most common errors
        # Does NOT check for STGP constraints
        if isinstance(key, slice):#key is object, slice is class
    ...
    def __str__(self):
        ...
        for node in self:
            stack.append((node, []))
            while len(stack[-1][1]) == stack[-1][0].arity:#[-1]で最後の要素
                prim, args = stack.pop()#リストの最後の要素である(node, [])を取り出してprimとargsに渡す
                string = prim.format(*args)#*argsですべての引数を受けてくれる
                if len(stack) == 0:
...
class Ephemeral(Terminal):
    ...
    """
    def __init__(self):
        Terminal.__init__(self, self.func(), symbolic=False, ret=_type_)
...
class PrimitiveSetTyped(object):
        ...
        def addType(dict_, ret_type):
            if not ret_type in dict_:
                ...
                for type_, list_ in list(dict_.items()):
                    if issubclass(type_, ret_type):#引数が正しいかチェック
                        for item in list_:
    ...
    def addEphemeralConstant(self, name, ephemeral, ret_type):
        ...
        """
        module_gp = globals() #今使えるグローバル変数をまとめたもの
...
class PrimitiveSet(PrimitiveSetTyped):
    ...
    def addPrimitive(self, primitive, arity, name=None):
        ...
        args = [__type__] * arity #type型のlistをarity分作る
